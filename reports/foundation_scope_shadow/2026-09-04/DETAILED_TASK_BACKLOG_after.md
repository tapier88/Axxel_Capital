...
### DATA-SCOPE-001 — FOUNDATION_SCOPE_READINESS_V1

**2026-09-04: SHADOW_IMPLEMENTED under approved ADR-FOUNDATION-001.**
Existing Data Engine now validates versioned scope manifests and authorizes before
artifact/hash/path/cache I/O; derivatives inherit intersections. Synthetic tests
cover ALLOW/DENY, parent revocation and strict research/execution separation.
Evidence: `reports/foundation_scope_shadow/2026-09-04/FOUNDATION_REPORT.md`.
G1 passes the implementation/test portion; G5/G6 pass SHADOW fixtures only.
No scientific certificate, production consumer migration or source promotion.

**Pending, not authorized for execution here:** review this evidence; define and
approve the Track B read-only server/account/T0 scope and preregister operational
parameters before worker implementation/capture. G3 historical route and G4 working
forward capture remain unmet. Legacy evidence blockers remain open. Further work
needs its own bounded authorization; do not start Control Tower.
